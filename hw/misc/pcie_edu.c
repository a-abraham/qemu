...
dev_sync(void)
{
    ...
    new_clock = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);
    timer_mod(ns.timer, new_clock + 10000);
    delta = ((new_clock - prev_clock) >> 8) + 1;
    delta = (delta < MAX_DELTA) ? delta : MAX_DELTA;
    prev_clock = new_clock;

    ns.rtc += delta;
#if 0
    if (pending_reply) {
        printf("%s: attempt to send pending reply...\n", __FUNCTION__);
        if (s_put_pkt(s_ptr, &ns.packet) < 0) return 0;
        pending_reply = 0;
        printf("%s: pending reply sent\n", __FUNCTION__);
        fflush(stdout);
    }
    while (s_get_pkt(s_ptr, &ns.packet) == 0) {
        if (ns.packet.reply) {
            if (ns.reply_ready) {
                printf("Oops! Second Aries reply\n");
                fflush(stdout);
                break;
            }
            ns.reply_packet = ns.packet;
            ns.reply_ready  = 1;
            break;
        }
        if (ns.packet.devfn == SNIF_REQ_TYPE_READ) {
            cpu_physical_memory_rw(ns.packet.addr, (uint8_t *)&ns.packet.data,
                                   ns.packet.bytes, false);
            ns.packet.reply = 1;
            if (s_put_pkt(s_ptr, &ns.packet) < 0) {
                pending_reply = 1;
                break;
            }
        } else if (ns.packet.devfn == SNIF_REQ_TYPE_WRITE) {
            cpu_physical_memory_rw(ns.packet.addr, (uint8_t *)&ns.packet.data,
                                   ns.packet.bytes, true);
                break;
        } else if (ns.packet.devfn == SNIF_REQ_TYPE_INTERRUPT) {
            /* For now we will hardcode the Physical Function here and disable
             * MSIX for VFs. Determine proper behavior TODO */
            msix_notify(physfn, ns.packet.irq);
        }
    }
#endif
    return 0;
...
static void pcie_edu_pci_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    ns.rtc = 1;
    ns.timer = timer_new_ns(QEMU_CLOCK_VIRTUAL, qtcb, NULL);
...
static void pcie_edu_class_init(ObjectClass *class, void *data)
{
    ...
    c->vendor_id = 0x000017db;   /* Cray Vendor ID */
